...
class GeoAnalyzer:
    ...
    def __init__(self, df):
        ...
        self.df_geo = self.df[
            ...
            (self.df['经纬度'].notna())
                ...
                # 提取经纬度
        self.df_geo['纬度'] = self.df_geo['经纬度'].apply(
            lambda x: float(x[0]) if isinstance(x, list) and len(x) > 0 and float(x[0]) != 0 else None
        )
        self.df_geo['经度'] = self.df_geo['经纬度'].apply(
            lambda x: float(x[1]) if isinstance(x, list) and len(x) > 1 and float(x[1]) != 0 else None
        )
        
        # 过滤掉经纬度为0或空的数据
        self.df_geo = self.df_geo[
            (self.df_geo['纬度'].notna()) & 
            (self.df_geo['经度'].notna()) &
            (self.df_geo['纬度'] != 0) &
            (self.df_geo['经度'] != 0)
        ].copy()
    ...
    def city_district_analysis(self, city_keyword, save_path=None):
        ...
        district_counts = df_city['城市'].value_counts().head(15)
        ...
        # 2. 区域平均薪资
        ax2 = axes[0, 1]
        district_key = '城市'# if '区域' in df_city.columns else '区县'
